...
class DistHandler(BaseHandler):
    ...
    def target_file(self) -> Path:
        return self.tools_dir / "distribution_system.py"
    
    def get_operations(self) -> dict:
        return {
            "status": "System-Status (Siegel, Mode, Dateien)",
            "verify": "Siegel-Integritaet pruefen",
            "scan": "Dateien scannen und registrieren",
    ...
    def _get_dist_system(self):
        """Lazy-load des Distribution-Systems."""
        if self._dist_system is None:
            try:
                from distribution_system import DistributionSystem
                self._dist_system = DistributionSystem()
            except ImportError as e:
                return None, f"[ERR] Konnte DistributionSystem nicht laden: {e}"
    ...
    def handle(self, operation: str, args: list, dry_run: bool = False) -> tuple:
        ...
        if op == "status":
            ...
            return self._scan_filesystem(args, dry_run)
        elif op == "snapshot":
    ...
    def _show_help(self) -> str:
        ...
        lines = [
            ...
            "  bach --dist status              System-Status anzeigen",
            "  bach --dist verify              Siegel pruefen",
            "  bach --dist scan                Dateien scannen/registrieren",
            ...
            "",
            "Optionen:",
            "  --dry-run    Nur simulieren (bei snapshot/release/scan)",
            "  --verbose    Ausfuehrliche Ausgabe (bei scan)",
            ""
        ]
        return "\n".join(lines)
    ...
    def _show_status(self) -> tuple:
        ...
        try:
            ...
            seal_icon = "✓" if seal.get("intact") else "✗"
            lines.append(f" Siegel: {seal_icon} {seal.get('message', 'Unbekannt')}")
            
            # Mode
            lines.append(f" Modus: {status.get('mode', 'unbekannt')}")
            
            # Statistiken
            lines.append(f" Dateien: {status.get('files', 0)}")
            lines.append(f" Versionen: {status.get('versions', 0)}")
            lines.append(f" Releases: {status.get('releases', 0)}")
            
            # Tier-Statistiken
            tiers = status.get("tiers", {})
            if tiers:
                tier_names = {0: "Kernel", 1: "Core", 2: "Extension", 3: "UserData"}
                tier_info = ", ".join(f"{tier_names.get(k, k)}={v}" for k, v in sorted(tiers.items()))
                lines.append(f" Tiers: {tier_info}")
            ...
            if identity:
                ...
                if inst:
                    ...
                    lines.append(f" ID: {inst.get('id', 'unbekannt')[:30]}...")
    ...
    def _scan_filesystem(self, args: list, dry_run: bool) -> tuple:
        """Scannt Dateisystem und registriert Dateien in DB."""
        verbose = "--verbose" in args or "-v" in args
        
        if dry_run:
            return True, "[DRY-RUN] Wuerde Filesystem scannen und Dateien registrieren"
        ...
        try:
            stats = dist.scan_filesystem(verbose=verbose)
            
            lines = [
                "",
                "[FILESYSTEM SCAN ABGESCHLOSSEN]",
                "=" * 50,
                f"  Hinzugefuegt:  {stats.get('added', 0)}",
                f"  Aktualisiert:  {stats.get('updated', 0)}",
                f"  Uebersprungen: {stats.get('skipped', 0)}",
            ...
            for tier_id, name in tier_names.items():
                count = by_tier.get(tier_id, 0)
                lines.append(f"  [{tier_id}] {name:12} {count}")
            
            lines.append("=" * 50)
            return True, "\n".join(lines)
            
        except Exception as e:
            return False, f"[ERR] Filesystem-Scan fehlgeschlagen: {e}"
    ...
    def _list_items(self, args: list) -> tuple:
        ...
        try:
            if item_type.startswith("snap"):
                ...
                # Releases - falls list_releases existiert
                return True, "[INFO] Release-Liste noch nicht implementiert"
