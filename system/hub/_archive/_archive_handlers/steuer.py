...
"""
    ...
    def _status(self, args: list) -> tuple:
        ...
        if not self.steuer_dir.exists():
            return True, "Steuer-Verzeichnis nicht gefunden.\nErstelle mit: bach steuer init YYYY"
    ...
    def _init_year(self, args: list, dry_run: bool) -> tuple:
        ...
        (jahr_dir / "belege").mkdir(parents=True)
        ...
        if self.templates_dir.exists():
            for template in self.templates_dir.glob("*.txt"):
                ...
                (jahr_dir / template.name).write_text(content, encoding='utf-8')
        else:
            # Fallback: Minimale Dateien erstellen
            self._create_minimal_files(jahr_dir, jahr, user)
        
        return True, f"Steuerjahr {jahr} erstellt.\nPfad: {jahr_dir}\n\nNaechste Schritte:\n1. Belege in {jahr_dir / 'belege'} ablegen\n2. Chat-Agent starten zur Erfassung"
  ...
  bach steuer export --jahr 2025 --format datev"""
    ...
    def _export_datev(self, jahr: str, liste: str = None) -> tuple:
        ...
        try:
            ...
            # Posten abfragen
            sql = """SELECT posten_id_str, bezeichnung, datum, brutto, netto, 
    ...
    def _export_csv(self, jahr: str, liste: str = None) -> tuple:
        ...
        try:
            ...
            sql += " ORDER BY liste, datum, belegnr"
    ...
    def _posten_list(self, args: list) -> tuple:
        ...
        try:
            ...
            # Query bauen
            sql = """SELECT posten_id_str, bezeichnung, brutto, liste, 
            ...
            sql += f" ORDER BY belegnr, postennr LIMIT {int(limit)}"
            ...
            results.append("Filter:  --liste W|G|V|Z  --belegnr NR  --limit N")
    ...
    def _posten_show(self, args: list) -> tuple:
        ...
        try:
            ...
            row = conn.execute("""
                ...
                WHERE posten_id_str = ? AND username = ?
            """, (posten_id, self.username)).fetchone()
            conn.close()
            ...
            results.append(f"Belegnr:          B{row['belegnr']:04d}")
            results.append("")
  ...
  --anbieter NAME    Anbieter (default: aus Beleg)"""
        ...
        try:
            ...
            conn.execute("""
                ...
                (username, steuerjahr, postennr, bezeichnung, datum, typ, 
                 rechnungssteller, dateiname, netto, brutto, liste, anteil,
                 absetzbar_netto, absetzbar_brutto, bemerkung, belegnr, 
                 posten_id_str, version, created_at, updated_at)
                VALUES (?, 2025, ?, ?, ?, 'Material', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, datetime('now'), datetime('now'))
            """, (self.username, next_nr, bezeichnung, beleg_datum,
                  anbieter or beleg['anbieter'], beleg['dateiname'],
                  netto_val, brutto_val, liste_full, anteil_val,
                  netto_val * anteil_val, absetzbar, bemerkung, int(belegnr), posten_id_str))
            conn.commit()
            conn.close()
    ...
    def _posten_edit(self, args: list, dry_run: bool) -> tuple:
        ...
        if not any([bezeichnung, brutto, anteil, bemerkung]):
...
Optionen:
  ...
  --bemerkung TXT    Neue Bemerkung"""
        ...
        try:
            ...
            # Posten pruefen
            row = conn.execute("""
                SELECT * FROM steuer_posten WHERE posten_id_str = ? AND username = ?
            """, (posten_id, self.username)).fetchone()
            ...
            params.extend([posten_id, self.username])
            conn.execute(f"""
  ...
  Z = ZURUECKGESTELLT (noch zu klaeren)"""
        ...
        try:
            ...
            conn.execute("""
                ...
                WHERE posten_id_str = ? AND username = ?
            """, (liste_full, anteil_val, absetzbar, absetzbar/1.19, posten_id, self.username))
            conn.commit()
            conn.close()
...
Bestaetigen mit: bach steuer posten delete {posten_id} --force"""
            ...
            conn.execute("""
                DELETE FROM steuer_posten WHERE posten_id_str = ? AND username = ?
            """, (posten_id, self.username))
            conn.commit()
            conn.close()
...
Die Belege werden als DEPRECATED markiert, die Nummern bleiben reserviert."""
    ...
    def _beleg_list(self, args: list) -> tuple:
        ...
        try:
            ...
            sql = """SELECT belegnr, dateiname, anbieter, erfasst_am, status, bemerkung
            ...
            sql += f" ORDER BY belegnr LIMIT {int(limit)}"
                ...
                if dry_run:
                    ...
                    conn.execute("""
                        ...
                        (username, steuerjahr, postennr, bezeichnung, datum, typ, 
                         ...
                         posten_id_str, version, created_at, updated_at)
                        VALUES (?, 2025, ?, ?, ?, 'Material', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, datetime('now'), datetime('now'))
                    """, (self.username, next_nr, bez, datum,
                          beleg['anbieter'], beleg['dateiname'],
                          netto_val, brutto_val, liste_full, anteil,
                          netto_val * anteil, absetzbar, bem, int(belegnr), posten_id_str))
...
"""
        ...
        for beleg in belege_data:
            ...
            json_str = json.dumps(posten_list)
...
"""
        ...
        try:
            ...
            if posten_ids:
                ...
                ids = [p.strip() for p in posten_ids.split(",") if p.strip()]
                placeholders = ",".join(["?" for _ in ids])
                rows = conn.execute(f"""
                    SELECT posten_id_str, bezeichnung, brutto, liste FROM steuer_posten
                    WHERE posten_id_str IN ({placeholders}) AND username = ?
                """, ids + [self.username]).fetchall()
            ...
            if posten_ids:
                ...
                liste_full = liste_map.get(liste.upper(), liste.upper())
                # IDs sammeln fuer gezieltes Loeschen
                ids_to_delete = [r['posten_id_str'] for r in rows]
                placeholders = ",".join(["?" for _ in ids_to_delete])
                conn.execute(f"""
                    DELETE FROM steuer_posten 
                    WHERE posten_id_str IN ({placeholders}) AND username = ?
                """, ids_to_delete + [self.username])
...
"""
        ...
        try:
            ...
            # Verschieben
            ids_to_move = [r['posten_id_str'] for r in rows]
            
            for pid in ids_to_move:
                # Brutto holen
                row = conn.execute("""
                    SELECT brutto FROM steuer_posten WHERE posten_id_str = ? AND username = ?
                """, (pid, self.username)).fetchone()
                ...
                if row:
                    ...
                    conn.execute("""
                        ...
                        SET liste = ?, anteil = ?, absetzbar_brutto = ?, 
                            absetzbar_netto = ?, updated_at = datetime('now')
                        WHERE posten_id_str = ? AND username = ?
                    """, (nach_full, anteil_val, absetzbar, absetzbar/1.19, pid, self.username))
    ...
    def _check(self, args: list) -> tuple:
        ...
        try:
            ...
            belege_ohne_posten = conn.execute("""
                SELECT d.belegnr, d.dateiname
                FROM steuer_dokumente d
                LEFT JOIN steuer_posten p ON d.belegnr = p.belegnr AND d.username = p.username
            ...
            if belege_ohne_posten:
                ...
                for b in belege_ohne_posten[:5]:
                    issues.append(f"  - B{b['belegnr']:04d}: {b['dateiname']}")
            ...
            posten_ohne_beleg = conn.execute("""
                SELECT p.posten_id_str, p.bezeichnung
                FROM steuer_posten p
                LEFT JOIN steuer_dokumente d ON p.belegnr = d.belegnr AND p.username = d.username
            ...
            # 3. Luecken in Belegnummern
            alle_belegnr = conn.execute("""
                SELECT DISTINCT belegnr FROM steuer_dokumente
                WHERE username = ? AND steuerjahr = ?
                ORDER BY belegnr
            """, (self.username, int(jahr))).fetchall()

            if alle_belegnr:
                nummern = [b['belegnr'] for b in alle_belegnr]
                luecken = []
  ...
  - Trinkgelder bei Geschaeftsessen"""
        ...
        try:
            ...
            max_eigenbeleg = conn.execute("""
                SELECT MAX(belegnr) as max_nr FROM steuer_posten
  ...
  Grund:       {grund}"""
            ...
            conn.execute("""
                ...
                (username, steuerjahr, postennr, bezeichnung, datum, typ,
                 ...
                 belegnr, posten_id_str, ist_eigenbeleg, version, created_at, updated_at)
                VALUES (?, ?, 1, ?, ?, 'Eigenbeleg',
                        'EIGENBELEG', ?, ?, ?, ?, ?,
                        ?, 1.0, ?, ?, ?,
                        ?, ?, 1, 1, datetime('now'), datetime('now'))
            """, (self.username, steuerjahr, bezeichnung, datum,
                  f"EIGENBELEG_{next_belegnr}.pdf", netto_val, brutto_val, mwst_betrag, mwst_satz_val,
                  liste_full, netto_val, brutto_val, grund,
                  next_belegnr, posten_id_str))
            conn.commit()
